...
class WeightProgressChart extends StatelessWidget {
  ...
  WeightProgressChart(this.entries);
...
class ChartPainter extends CustomPainter {
  ...
  void _drawBottomLabels(Canvas canvas, Size size) {
    for (int daysFromStart = WeightProgressChart.NUMBER_OF_DAYS;
    daysFromStart >= 0;
    daysFromStart -= 7) {
      double offsetXbyDay = drawingWidth / (WeightProgressChart.NUMBER_OF_DAYS);
  ...
  ui.Paragraph _buildParagraphForBottomLabel(int daysFromStart) {
    ui.ParagraphBuilder builder = new ui.ParagraphBuilder(
      ...
      ..addText(new DateFormat('d MMM').format(new DateTime.now().subtract(
          new Duration(days: WeightProgressChart.NUMBER_OF_DAYS - daysFromStart))));
  ...
  Offset _getEntryOffset(WeightEntry entry, DateTime beginningOfChart,
    ...
    int daysFromBeginning = entry.dateTime
        ...
        .inDays;
    double relativeXposition = daysFromBeginning / WeightProgressChart.NUMBER_OF_DAYS;
...
DateTime _getStartDateOfChart() {
  ...
  DateTime beginningOfChart = now.subtract(new Duration(
      days: WeightProgressChart.NUMBER_OF_DAYS,
      hours: now.hour,
      minutes: now.minute));
