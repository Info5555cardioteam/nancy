...
class GlucoseProgressChart extends StatelessWidget {
  static const int NUMBER_OF_DAYS = 31;
  final List<GlucoseEntry> entries;

  GlucoseProgressChart(this.entries);
  ...
  List<GlucoseEntry> _prepareEntryList(List<GlucoseEntry> initialEntries) {
    DateTime beginningDate = _getStartDateOfChart();
    List<GlucoseEntry> entries = initialEntries
  ...
  void _addFakeEntryOnTheChartBeginning(List<GlucoseEntry> initialEntries,
      List<GlucoseEntry> entries, DateTime beginningDate) {
    List<GlucoseEntry> entriesNotInChart =
    initialEntries.where((entry) => !entries.contains(entry)).toList();
    GlucoseEntry firstEntryAfterBeginning = entries.last;
    GlucoseEntry lastEntryBeforeBeginning = entriesNotInChart.first;
    GlucoseEntry fakeEntry = new GlucoseEntry(
        beginningDate,
  ...
  double _calculateWeightOnBeginningDate(GlucoseEntry lastEntryBeforeBeginning,
      GlucoseEntry firstEntryAfterBeginning, DateTime beginningDate) {
    ...
    double differenceInWeight =
        firstEntryAfterBeginning.glucose - lastEntryBeforeBeginning.glucose;
    ...
    double estimatedWeight =
        lastEntryBeforeBeginning.glucose + weightChangeFromLastEntry;
...
class ChartPainter extends CustomPainter {
  ...
  void _drawBottomLabels(Canvas canvas, Size size) {
    for (int daysFromStart = GlucoseProgressChart.NUMBER_OF_DAYS;
    daysFromStart >= 0;
    daysFromStart -= 7) {
      double offsetXbyDay = drawingWidth / (GlucoseProgressChart.NUMBER_OF_DAYS);
  ...
  ui.Paragraph _buildParagraphForBottomLabel(int daysFromStart) {
    ui.ParagraphBuilder builder = new ui.ParagraphBuilder(
      ...
      ..addText(new DateFormat('d MMM').format(new DateTime.now().subtract(
          new Duration(days: GlucoseProgressChart.NUMBER_OF_DAYS - daysFromStart))));
  ...
  Tuple2<int, int> _getMinAndMaxValues(List<GlucoseEntry> entries) {
    double maxWeight = entries.map((entry) => entry.glucose).reduce(math.max);
    double minWeight = entries.map((entry) => entry.glucose).reduce(math.min);
  ...
  Offset _getEntryOffset(GlucoseEntry entry, DateTime beginningOfChart,
      int minLineValue, int maxLineValue) {
    int daysFromBeginning = entry.dateTime
        .difference(beginningOfChart)
        .inDays;
    double relativeXposition = daysFromBeginning / GlucoseProgressChart.NUMBER_OF_DAYS;
    double xOffset = leftOffsetStart + relativeXposition * drawingWidth;
    double relativeYposition =
        (entry.glucose - minLineValue) / (maxLineValue - minLineValue);
...
DateTime _getStartDateOfChart() {
  ...
  DateTime beginningOfChart = now.subtract(new Duration(
      days: GlucoseProgressChart.NUMBER_OF_DAYS,
      hours: now.hour,
      minutes: now.minute));
